...
- Branding: update launchd labels, mobile bundle IDs, and logging subsystems to bot.molt (legacy com.moltbot migrations). Thanks @thewilloftheshadow.
...
- Doctor: honor MOLTBOT_GATEWAY_TOKEN for auth checks and security audit token reuse. (#1448) Thanks @azade-c.
...
- Exec approvals: migrate approvals to `~/.moltbot/exec-approvals.json` with per-agent allowlists + skill auto-allow toggle, and add approvals UI + node exec lifecycle events. https://docs.molt.bot/tools/exec-approvals
...
- Daemon: share profile/state-dir resolution across service helpers and honor `MOLTBOT_STATE_DIR` for Windows task scripts.
...
- Onboarding/Auth: honor `MOLTBOT_AGENT_DIR` / `PI_CODING_AGENT_DIR` when writing auth profiles (MiniMax). (#829) — thanks @roshanasingh4.
...
- Gateway/CLI: honor `MOLTBOT_LAUNCHD_LABEL` / `MOLTBOT_SYSTEMD_UNIT` overrides; `agents.list` respects explicit config; reduce noisy loopback WS logs during tests; run `moltbot doctor --non-interactive` during updates. (#781) — thanks @ronyrus.
...
- Installer UX: support `MOLTBOT_NO_ONBOARD=1` for non-interactive installs; fix npm prefix on Linux and auto-install git.
...
- Config: expand `~` in `MOLTBOT_CONFIG_PATH` and common path-like config fields (including `plugins.load.paths`); guard invalid `$include` paths. (#731) — thanks @pasogott.
...
- CLI: respect `MOLTBOT_STATE_DIR` for node pairing + voice wake settings storage. (#664) — thanks @azade-c.
...
- Gateway/Agent: accept image attachments on `agent` (multimodal message) and add live gateway image probe (`MOLTBOT_LIVE_GATEWAY_IMAGE_PROBE=1`).
