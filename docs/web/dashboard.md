...
- **Token source**: `gateway.auth.token` (or `MOLTBOT_GATEWAY_TOKEN`); the UI stores it after first load.
...
- In the dashboard settings, paste the same token you configured in `gateway.auth.token` (or `MOLTBOT_GATEWAY_TOKEN`).
