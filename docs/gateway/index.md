...
pnpm gateway:watch
```
- Config hot reload watches `~/.moltbot/moltbot.json` (or `MOLTBOT_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__moltbot__/canvas/` from `~/clawd/canvas`. Disable with `canvasHost.enabled=false` or `MOLTBOT_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `MOLTBOT_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `MOLTBOT_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- macOS: `bot.molt.<profile>` (legacy `com.moltbot.*` may still exist)
...
- `MOLTBOT_GATEWAY_PORT=19001` (Gateway WS + HTTP)
...
- `canvasHost.port = base + 4` (or `MOLTBOT_CANVAS_HOST_PORT` / config override)
...
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `moltbot gateway install` writes `~/Library/LaunchAgents/bot.molt.gateway.plist`
    (or `bot.molt.<profile>.plist`; legacy `com.moltbot.*` is cleaned up).
...
- Moltbot.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `bot.molt.gateway` (or `bot.molt.<profile>`; legacy `com.moltbot.*` labels still unload cleanly).
