...
private func resolvedToken(opts: WizardCliOptions, config: GatewayConfig) -> String? {
    ...
    if let token = ProcessInfo.processInfo.environment["MOLTBOT_GATEWAY_TOKEN"], !token.isEmpty {
        return token
    }
...
private func resolvedPassword(opts: WizardCliOptions, config: GatewayConfig) -> String? {
    ...
    if let password = ProcessInfo.processInfo.environment["MOLTBOT_GATEWAY_PASSWORD"], !password.isEmpty {
        return password
    }
    if (config.mode ?? "local").lowercased() == "remote" {
