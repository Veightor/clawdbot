...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.moltbot/moltbot.json. Prefer MOLTBOT_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.moltbot/moltbot.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.moltbot/moltbot.json. Restart the gateway to apply."
        }
