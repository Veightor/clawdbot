...
describe("state + config path candidates", () => {
  ...
  it("orders default config candidates as new then legacy", () => {
    ...
    expect(candidates[1]).toBe(path.join(home, ".moltbot", "moltbot.json"));
    expect(candidates[2]).toBe(path.join(home, ".moltbot", "moltbot.json"));
    expect(candidates[3]).toBe(path.join(home, ".moltbot", "moltbot.json"));
  });
